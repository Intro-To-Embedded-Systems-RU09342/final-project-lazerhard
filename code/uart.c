...
void uart_rx_execute_cmd_with_arg()
{
    ...
    if(cmd[0] == SET_C_CMD)
        speaker_set(0, arg);
    else if(cmd[0] == SET_D_CMD)
        speaker_set(1, arg);
    else if(cmd[0] == SET_E_CMD)
        speaker_set(2, arg);
    else if(cmd[0] == SET_F_CMD)
        speaker_set(3, arg);
    else if(cmd[0] == SET_G_CMD)
        speaker_set(4, arg);
    else if(cmd[0] == SET_A_CMD)
        speaker_set(5, arg);
    else if(cmd[0] == SET_B_CMD)
        speaker_set(6, arg);
    else if(cmd[0] == SET_ALL_CMD)
        speaker_set_all(arg);
}
